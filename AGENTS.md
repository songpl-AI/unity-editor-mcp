...
**修复流程：** 评估 → 修复代码 → 测试验证 → 更新 CHANGELOG → 清理临时文件

**文档要求：**
- ✅ **必须更新：** `CHANGELOG.md`（记录问题、原因、解决方案、验证方法）
- ✅ **复杂 Bug 可创建：** `bug-record/` 下的技术分析文档（核心功能失效、跨平台兼容性等）
- ❌ **禁止提交：** 临时测试脚本、验证文档、总结文档、检查清单、Debug 代码

### 测试规范

**必须进行测试，但区分两种类型：**

1. **临时验证** — 创建脚本 → 测试通过 → 在 CHANGELOG 中记录验证方法 → **立即删除脚本**
2. **长期测试** — 重构为规范测试 → 加入 `tests/` 目录 → 提交到仓库

**测试归宿：** 删除（一次性验证）或保留（加入 `tests/` 目录）

### 禁止提交的文件

❌ `test_*.sh`, `verify_*.py` — 一次性验证脚本（测试后删除）  
❌ `*_verification.md`, `fix_summary.md` — 验证/总结文档（应在 CHANGELOG 中）  
❌ `CHECKLIST.md`, `TODO.md` — 任务清单（使用 Git commit 或 Issue）  
❌ `test_*.cs`, `debug_*.py` — 临时调试代码（调试后删除）
...
### Fixed
- 修复参数化路由无法匹配的问题（RequestRouter.cs）
  - 根本原因：Regex.Escape() 不会转义冒号
  - 影响范围：5 个 GameObject 组件相关端点
  - 解决方案：更新正则表达式支持未转义的冒号
  - 验证方法：测试所有参数化路由，确认返回 HTTP 200
```
